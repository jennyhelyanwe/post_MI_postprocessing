%%%%%% This script is to simulate baseline or population of Type 1 acute stage
%%%%%% post infarction border zone cell electromechanics based on the ToR-ORd-Land model

clear
close all

mkdir AcuteBZ1endoPOM
...
% Parameters=importdata('POM45.mat'); %%%% parameters of population of
...
for i=1:size(Parameters,1)
    ...
    param.Ito_Multiplier=Parameters(i,4)*0.1;
    param.ICaL_Multiplier=Parameters(i,5)*0.64;
    param.IKr_Multiplier=Parameters(i,6)*0.7;
    param.IKs_Multiplier=Parameters(i,7)*0.2;
    param.IK1_Multiplier=Parameters(i,8)*0.3;
    ...
    filename=sprintf('%s%d%s%d%s','AcuteBZ1endoCL',param.bcl,'Model',index,'.mat');
    cd AcuteBZ1endoPOM
