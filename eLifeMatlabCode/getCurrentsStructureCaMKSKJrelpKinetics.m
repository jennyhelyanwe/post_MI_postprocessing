function currents = getCurrentsStructureCaMKSKJrelpKinetics(time, X, parameters, ignoreFirstSpikes)
...
if(~isfield(parameters,'model'))
    parameters.model = @model_ToRORd_LandCaMKSKJrelpKinetics;%%%% changed from model0
end
...
if(~isfield(parameters,'model'))
    parameters.model = @modelmodel_ToRORd_LandCaMKSKJrelpKinetics;%%%% changed from @model
end
...
currents.Bcai = cell2mat(time((ignoreFirstSpikes+1):end));

currents.IKCa = zeros(nPoints, 1);
currents.Ta = zeros(nPoints, 1);
...
for iBeat = 1:size(time, 1)
    ...
    for j=1:size(X{iBeat},1)
        ...
        if (length(IsJs) >=33) 
            ...
            currents.BCai(i) = IsJs(37);
        end
        
        currents.IKCa(i) = IsJs(38);
        currents.Ta(i) = IsJs(39);
